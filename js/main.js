...
function getResults() {
	...
	if (
		...
		for (let i = 0; i < recipesfusion.length; i++) {
			sortRecette.push(recipesfusion[i]);
		}
		...
		for (let i = 0; i < recipesfusion.length; i++) {
			if (search.length >= 3) {
				...
				fermetureListeAppareil();
				fermetureListeUstensiles();
				for (var a = 0; a < recipesfusion[i].ingredients.length; a++) {
					if (
						recipesfusion[i].ingredients[a].ingredient
							.toLocaleLowerCase()
							.includes(search.toLocaleLowerCase()) ||
						recipesfusion[i].name
							.toLocaleLowerCase()
							.includes(search.toLocaleLowerCase()) ||
						recipesfusion[i].description
							.toLocaleLowerCase()
							.includes(search.toLocaleLowerCase())
					) {
						if (tagIngredients.length >= 1) {
							//ingredient
							sortRecette.push(mergeSort(recipesfusion[i]));
